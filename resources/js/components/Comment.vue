    ...
    <div class="border rounded shadow-md mb-2">
        <div class="border-b">
            <div class="py-1 px-3 flex gap-1 items-baseline">
                ...
                <form v-if="!comment.deleted && (privileges.owned || privileges.userId == comment.user_id)" action="/profile/delete-comment" method="POST" class="ml-auto">
        ...
        <div class="py-0.5 px-3 flex items-baseline">
            ...
            <button v-if="privileges.auth && privileges.userId != comment.user_id" @click="showForm = !showForm" class="ml-auto text-sm">{{ !showForm ? 'Reply' : 'Hide' }}</button>
        </div>
    </div>
    <div v-if="privileges.auth && privileges.userId != comment.user_id" class="mb-2">
        <add-comment-form v-show="showForm" :comment-id="comment.parent_id"/>
    </div>
